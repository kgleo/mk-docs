𝐏𝐫𝐨𝐦𝐩𝐭 → 𝐁𝐏𝐌𝐍 𝟐.𝟎 𝐃𝐢𝐚𝐠𝐫𝐚𝐦 (𝐢𝐧 𝐦𝐢𝐧𝐮𝐭𝐞𝐬)  

Here’s the playbook I use:  
  
1) Describe your process like a BA (not like a developer)  
2) Start with roles, steps, decisions, outcomes, and exceptions.  
3) Example process (simple & real): “Leave Request Approval”   
 - Employee submits leave request  
 - Manager reviews  
 - If approved → notify employee → end  
 - If not approved → send back for revision → resubmit → review again  
  
1) Prompt AI to create BPMN 2.0 XML (with DI)  
Ask for:  
```
